...
read -rp "Enter your Tastytrade account number: " TASTYTRADE_ACCOUNT_NUMBER
if [ -z "$TASTYTRADE_ACCOUNT_NUMBER" ]; then
    echo "[ERROR] Tastytrade account number cannot be empty."
    exit 1
fi

read -rsp "Enter your Tastytrade OAuth client secret: " TASTYTRADE_CLIENT_SECRET
echo
if [ -z "$TASTYTRADE_CLIENT_SECRET" ]; then
    echo "[ERROR] Tastytrade client secret cannot be empty."
    exit 1
fi

read -rsp "Enter your Tastytrade OAuth refresh token: " TASTYTRADE_REFRESH_TOKEN
echo
if [ -z "$TASTYTRADE_REFRESH_TOKEN" ]; then
...
TASTYTRADE_ENTRY_TIME_IN_FORCE=Day
TASTYTRADE_EXIT_TIME_IN_FORCE=GTC
TASTYTRADE_STOP_ORDER_TYPE=StopLimit
ALPACA_API_KEY=
ALPACA_SECRET_KEY=
